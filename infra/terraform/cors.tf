...
# Enables cross-origin requests from the dashboard (CloudFront or localhost).

locals {
  # All API resources that need OPTIONS methods
  cors_resources = {
    projects          = aws_api_gateway_resource.projects.id
    status            = aws_api_gateway_resource.status.id
    deliverables      = aws_api_gateway_resource.deliverables.id
    approve           = aws_api_gateway_resource.approve.id
    revise            = aws_api_gateway_resource.revise.id
    interrupt_respond = aws_api_gateway_resource.interrupt_respond.id
...
# --- Gateway Responses (CORS headers on 4XX/5XX errors) ---
...
# --- OPTIONS methods (preflight) for each resource ---
